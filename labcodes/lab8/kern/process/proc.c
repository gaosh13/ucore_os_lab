...
alloc_proc(void) {
     ...
     */
     //LAB5 2013011400: (update LAB4 steps)
    /*
     * below fields(add in LAB5) in proc_struct need to be initialized
     *       uint32_t wait_state;                        // waiting state
     *       struct proc_struct *cptr, *yptr, *optr;     // relations between processes
	 */
     //LAB6 2013011400: (update LAB5 steps)
    /*
     ...
     */
     //LAB8 2013011400 HINT: add filesp
...
do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf) {
    ...
    ret = -E_NO_MEM;
    //LAB4:EXERCISE2 2013011400
    //LAB8:EXERCISE2 2013011400 HINT:how to copy the fs in parent's proc_struct?
    /*
...
load_icode(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 2013011400 HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    struct elfhdr temp_elf, *elf = &temp_elf;
    if (ret = load_icode_read(fd, elf, sizeof(struct elfhdr), 0)) {
    ...
    struct proghdr temp_prog, *prog = &temp_prog;
    ...
    for (phnum = 0; phnum < elf->e_phnum; phnum ++)
    {
        ...
        if (ret = load_icode_read(fd, prog, sizeof(struct proghdr), phoff))
        ...
        if (ret = mm_map(mm, prog->p_va, prog->p_memsz, vm_flags, NULL))
        {
            goto bad_cleanup_mmap;
        }

        off_t offset = prog->p_offset;
        size_t off, size;
        uintptr_t start = prog->p_va, end, la = ROUNDDOWN(start, PGSIZE);

        for (end = prog->p_va + prog->p_filesz; start < end; start += size)
        {
    ...
    uint32_t asize = 0, i;
    for (i = 0; i < argc; i++)
        asize += strnlen(kargv[i], EXEC_MAX_ARG_LEN + 1) + 1;
    uintptr_t stop = USTACKTOP - (asize / sizeof(long) + 1) * sizeof(long);
    char** uargv = (char**)(stop - argc * sizeof(char*));
    asize = 0;
    for (i = 0; i < argc; i++)
    {
        uargv[i] = strcpy((char*)(stop + asize), kargv[i]);
        asize += strnlen(kargv[i], EXEC_MAX_ARG_LEN + 1) + 1;
    }
    stop = (uintptr_t)uargv - sizeof(int);
    *(int*)stop = argc;
